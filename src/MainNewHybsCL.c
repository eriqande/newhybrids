...
GetNewHybsOPTS(char **argv, int argc, hyb_data *D,hyb_prior *P,hyb_chain *C, long* Seed1, long *Seed2)
{

	int DataFileNameFlag = 0,
		GtypCatsFlab = 0,
		MixPropFlag = 0,
		AlleFreqFlag = 0,
		SeedFlag = 0;
	...
	*Seed2 = 0;
	...
	BEGIN_OPT_LOOP
		...
		/* data file name */
		if( REQUIRED_OPTION(DataFileNameFlag,
				d,
				data-file,
				[S],
				[S]=name of the data file,
				Follow this option with the pathname to the data file.  The format of the data file
				is described in documentation.  Will fill in more later. Required.) ) {
			
		}
		
		/* genotype freq categories */
...
{	
	char File[10000], DatFile[10000], GtypFile[10000];
	int temp;
	double temp_d;
	...
	switch(priorchoice) {
		...
		case(1):
			...
			break;
		case(2):
			PiP=JEFFREYS;  /* I have to make a new enum for this that reports it properly */
			gPiFixed = 1; 
